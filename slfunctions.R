mySL <- function (Y, X, newX = NULL, family , SL.library,
...
screen.corRank50 <- function(Y, X, family, obsWeights, id, method = "pearson", rank = 50, ...)
    screen.corRank(Y, X, family, obsWeights, id, method = method, rank = rank)

SL.myglmnet <- function (Y, X, newX, family, obsWeights, id, alpha = 1, nfolds = 3, useMin = FALSE,  ...)

{

    .SL.require("glmnet")

        formula <- as.formula('~ -1 + .^2')
    ...
    fitCV <- glmnet::cv.glmnet(x = X, y = Y, weights = obsWeights,
        ...
        #KER bounding
    preda <- predict(fitCV, newdata = newX, type = "response", s = ifelse(useMin,

        "lambda.min", "lambda.1se"))

    pred <- pmax(preda, 0.01)
...

{
    ...
    #KER bounding
    preda <- predict(object$object, newdata = newX, type = "response",
